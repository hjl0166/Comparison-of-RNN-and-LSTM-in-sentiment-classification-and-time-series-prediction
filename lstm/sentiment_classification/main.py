...
for epoch in range(epochs):
    ...
    if (Best_Acc < Accuracy):
        ...
        torch.save(best_model_weights, 'runs/exp2/best.pth')  # 保存权重到指定路径
...
loss_df.to_csv('runs/exp2/result.csv', index=False, encoding='utf-8-sig')  # 保存为 UTF-8 编码，防止中文乱码
...
with torch.no_grad():
    ...
    for texts, labels in test_loader:
        ...
        # h = h.to(device)  # 确保 hidden state 也在相同设备
        outputs, h = model(texts, h)
        predicted = (outputs > 0.5).float()
        correct += (predicted == labels).sum().item()
        total += labels.size(0)
